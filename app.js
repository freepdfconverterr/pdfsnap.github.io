...
setTimeout(function(){if(typeof openTool==='function')openTool(toolParam); }, 400);
});
}
})();
...
var _pdfLibsCallbacks=null;
function ensurePdfLibs(cb){
if(libsOK){if(cb)cb(); return; }
if(_pdfLibsCallbacks===true){if(cb)cb(); return; }
if(Array.isArray(_pdfLibsCallbacks)){_pdfLibsCallbacks.push(cb); return; }
...
var O=_rc4(_md5(ownerPad).slice(0,5), userPad);
...
var U=_rc4(eKey, PAD.slice()).concat(new Array(16).fill(0));
